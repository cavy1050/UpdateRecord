--门诊账户抵用记录库(日表)
if not exists(select 1 from sysobjects where name='YY_CQYB_MZDYJLK')
begin  
	create table YY_CQYB_MZDYJLK
	(      
		...
		constraint PK_YY_CQYB_MZDYJLK primary key(jssjh,dykh)
	)
	create index idx_jssjh on YY_CQYB_MZDYJLK(jssjh)	
	create index idx_sbkh on YY_CQYB_MZDYJLK(sbkh)
end
go

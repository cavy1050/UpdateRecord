--医保住院结算记录费用明细信息(日表)
if not exists(select 1 from sysobjects where name='YY_CQYB_ZYFYMXK_JS')
begin
	create table YY_CQYB_ZYFYMXK_JS
	(   
		...
		constraint PK_YY_CQYB_ZYFYMXK_JS primary key(xh,jsxh)
	)
	create index idx_syxh on YY_CQYB_ZYFYMXK_JS(syxh)
	create index idx_jsxh on YY_CQYB_ZYFYMXK_JS(jsxh)
	create index idx_xh on YY_CQYB_ZYFYMXK_JS(xh)
	create index idx_txh on YY_CQYB_ZYFYMXK_JS(txh)
	create index idx_cfh on YY_CQYB_ZYFYMXK_JS(cfh)
end;
GO

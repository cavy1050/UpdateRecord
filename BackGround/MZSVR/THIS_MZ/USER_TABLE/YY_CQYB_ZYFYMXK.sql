...
begin
	...
	(   
		...
		ybscbz			ut_bz			null,	--医保上传标志0未上传1已上传2无需上传 3不上传（已正负冲销）
		      			     			     	--4部分退后需红冲已上传  5 红冲后待重新上传和部分退后上传				
...
BEGIN
	ALTER TABLE YY_CQYB_ZYFYMXK ADD dydm  VARCHAR(32)     null    --上传的医保流水号
	ALTER TABLE YY_CQYB_NZYFYMXK ADD dydm  VARCHAR(32)     null    --上传的医保流水号
end

IF NOT EXISTS (SELECT 1 FROM syscolumns WHERE id = OBJECT_ID('YY_CQYB_ZYFYMXK') AND name = 'scxmdj') 
	ALTER table YY_CQYB_ZYFYMXK add scxmdj ut_money NULL --上传单价
IF NOT EXISTS (SELECT 1 FROM syscolumns WHERE id = OBJECT_ID('YY_CQYB_ZYFYMXK') AND name = 'scxmsl') 
	ALTER table YY_CQYB_ZYFYMXK add scxmsl ut_sl10 NULL --上传数量
IF NOT EXISTS (SELECT 1 FROM syscolumns WHERE id = OBJECT_ID('YY_CQYB_ZYFYMXK') AND name = 'scxmje') 
	ALTER table YY_CQYB_ZYFYMXK add scxmje ut_money NULL --上传金额
--医保门诊结算信息(日表)
if not exists(select 1 from sysobjects where name='YY_CQYB_MZJSJLK')
begin  
	create table YY_CQYB_MZJSJLK
	(      
		...
		constraint PK_YY_CQYB_MZJSJLK primary key(jssjh)
	)
	create index idx_jssjh on YY_CQYB_MZJSJLK(jssjh)	
	create index idx_sbkh on YY_CQYB_MZJSJLK(sbkh)	
...
--定点医疗机构编码
if not exists(select 1 from syscolumns where id=object_id('YY_CQYB_MZJSJLK') and name='ddyljgbm')
	alter table YY_CQYB_MZJSJLK add ddyljgbm varchar(20) null
go
--医保住院费用明细信息(年表)
if not exists(select 1 from sysobjects where name='YY_CQYB_NZYFYMXK')
begin
	create table YY_CQYB_NZYFYMXK
	(   
		...
		constraint PK_YY_CQYB_NZYFYMXK primary key(xh)
	)
	create index idx_syxh on YY_CQYB_NZYFYMXK(syxh)
	create index idx_jsxh on YY_CQYB_NZYFYMXK(jsxh)
	create index idx_xh on YY_CQYB_NZYFYMXK(xh)
	create index idx_txh on YY_CQYB_NZYFYMXK(txh)
	create index idx_cfh on YY_CQYB_NZYFYMXK(cfh)
end;
go

IF NOT EXISTS (SELECT 1 FROM syscolumns WHERE id = OBJECT_ID('YY_CQYB_NZYFYMXK') AND name = 'scxmdj') 
	ALTER table YY_CQYB_NZYFYMXK add scxmdj ut_money NULL --上传单价
IF NOT EXISTS (SELECT 1 FROM syscolumns WHERE id = OBJECT_ID('YY_CQYB_NZYFYMXK') AND name = 'scxmsl') 
	ALTER table YY_CQYB_NZYFYMXK add scxmsl ut_sl10 NULL --上传数量
IF NOT EXISTS (SELECT 1 FROM syscolumns WHERE id = OBJECT_ID('YY_CQYB_NZYFYMXK') AND name = 'scxmje') 
	ALTER table YY_CQYB_NZYFYMXK add scxmje ut_money NULL --上传金额
--已对照项目
if exists(select 1 from sysobjects where type='V' and name='VW_YPXMMATCHED')
begin
    DROP VIEW VW_YPXMMATCHED
end
go
...
SET ANSI_NULLS OFF
GO
CREATE  VIEW VW_YPXMMATCHED 
    (    
      hisxmdm ,hisxmmc ,ypdm ,hisypjx ,hisypgg ,hisdw ,hisdj ,hisbzsl ,hiscd ,    
      ybdxmdm ,yblsh ,ybxmdm ,ybxmmc ,ybypjx ,dybz ,fydj ,sflb ,py ,xmzl ,    
      isdy ,ypbz1 ,zfbl ,isyp ,ybypgg ,ybypcj ,ybfydj ,dxmdm ,zyfp_id,ybdj,memo,ypbz,tybz
    )    
AS    
    SELECT  CONVERT(VARCHAR(12), a.idm) ,    
            a.ypmc + '[' + '本地单价：' + CONVERT(VARCHAR(20), a.ylsj) + ']' + '['    
            + '医保单价：' + CONVERT(VARCHAR(20), c.ylbzdj) + ']' ,    
            a.ypdm ,    
            b.name ,    
            a.ypgg ,    
            a.ykdw ,    
    ...
    FROM    YK_YPCDMLK a(NOLOCK) LEFT OUTER JOIN VW_YBYPXM c(NOLOCK) ON a.dydm = c.yblsh   
								INNER JOIN YK_YPJXK b(NOLOCK) ON a.jxdm = b.id    
								INNER JOIN YY_SFDXMK d(NOLOCK) ON a.yplh = d.id    
    WHERE   a.isdy = 1 
    UNION ALL    
    SELECT  a.id ,    
            a.name + '[' + '本地单价：' + CONVERT(VARCHAR(20), a.xmdj) + ']' + '['    
            + '医保单价：' + CONVERT(VARCHAR(20), b.ylbzdj) + ']' ,    
            a.id ,    
            NULL ,    
            NULL ,    
            a.xmdw ,    
    ...
    FROM    YY_SFXXMK a(NOLOCK) LEFT JOIN VW_YBYPXM b(NOLOCK) ON  a.dydm = b.yblsh   
